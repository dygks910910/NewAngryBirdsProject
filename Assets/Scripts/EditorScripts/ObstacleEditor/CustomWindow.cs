...
namespace YH_CustomEditor
{
    ...
    public class CustomWindow : EditorWindow
    {
        ...
        string gameObjectName = "obstacle";
    ...
    public class CustomWindowEdit : EditorWindow
    {
        const int MAX_SPRITE_COUNT = 4;
        SerializedProperty m_IntProp;
        public GameObject referenceObject;
        ...
        int colliderSelected = 0;
        string gameObjectName = "obstacle";
        [MenuItem("YH_Custom/CreateObstacleWindowByObject")]
        static void CreateWindow()
        {
            CustomWindowEdit wnd = EditorWindow.CreateWindow<CustomWindowEdit>();
            wnd.Show();
            wnd.titleContent = new GUIContent("CreateObstacleWindow");
        }
        private void OnGUI()
        {
            ...
            CustomWndHelper.CreateGameObjectField("HPLow", ref sprite1);
            CustomWndHelper.CreateGameObjectField("HPMidle", ref sprite2);
            CustomWndHelper.CreateGameObjectField("HPMidleBig", ref sprite3);
            CustomWndHelper.CreateGameObjectField("HPBig", ref sprite4);
            //체크박스
            GUILayout.Space(10);
            GUILayout.Label("추가할 Component들");
            CustomWndHelper.CreateTextToggle("RigidBody", ref isUsingRigidBody);
        ...
        private void CreateObstacleObject()
        {
            ...
            tmpGameobj.tag = UnityEditorInternal.InternalEditorUtility.tags[tagIdx];
            SpriteRenderer tmpSpriteRenderer = tmpGameobj.AddComponent<SpriteRenderer>();
            tmpSpriteRenderer.sprite = sprite4.GetComponent<SpriteRenderer>().sprite;
            tmpSpriteRenderer.sortingLayerName = YH_Helper.YH_Helper.GetSortingLayerNames()[sortingLayerIdx];
            if (isUsingRigidBody)
            {
                Rigidbody2D rgd2D = tmpGameobj.AddComponent<Rigidbody2D>();
            ...
            if (isUsingObstacleInterationScripts)
            {
                obstacleInteration interation = tmpGameobj.AddComponent<obstacleInteration>();
                interation.sprites = new Sprite[4];
                interation.sprites[0] = sprite1.GetComponent<SpriteRenderer>().sprite;
                interation.sprites[1] = sprite2.GetComponent<SpriteRenderer>().sprite;
                interation.sprites[2] = sprite3.GetComponent<SpriteRenderer>().sprite;
                interation.sprites[3] = sprite4.GetComponent<SpriteRenderer>().sprite;
